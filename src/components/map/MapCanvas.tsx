...
export function MapCanvas({
  seats, activeTool, selectedSeatId,
  onSeatAdd, onSeatMove, onSeatResize, onSeatSelect, onSeatDoubleClick,
}: MapCanvasProps) {
  ...
  const draw = useCallback(() => {
    ...
    seats.forEach((seat) => {
      ...
      if (isSelected) {
        ...
        ctx.beginPath(); ctx.roundRect(hx, hy, 12, 12, 3); ctx.fill();
  ...
  function getSeatAt(x: number, y: number): Seat | null {
    for (let i = seats.length - 1; i >= 0; i--) {
      ...
      if (x >= s.position.x && x <= s.position.x + s.size.width &&
          y >= s.position.y && y <= s.position.y + s.size.height) return s;
    }
    return null;
  }
  ...
  function handleMouseDown(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (activeTool === "select") {
      if (seat) {
        ...
        const mode: DragMode =
          (seat.id === selectedSeatId && isInResizeHandle(seat, x, y)) ? "resize" : "move";
        drag.current = {
          id: seat.id, mode,
          startX: x, startY: y,
  ...
  function applyDrag(d: DragState, x: number, y: number) {
    ...
    const dy = y - d.startY;
    if (d.mode === "move") {
      onSeatMove(d.id, { x: snap(clamp(d.origX + dx, 0, 800)), y: snap(clamp(d.origY + dy, 0, 540)) });
    } else {
      onSeatResize(d.id, {
  ...
  function addSeat(x: number, y: number) {
    const typeMap: Record<string, Seat["type"]> = { addTable: "table", addBar: "bar", addBooth: "booth" };
    const capacityMap: Record<string, number> = { addTable: 4, addBar: 2, addBooth: 6 };
    ...
    const labelMap: Record<string, string> = { table: "テーブル", bar: "バー", booth: "ボックス" };
    onSeatAdd({
      id: `seat-${Date.now()}`,
